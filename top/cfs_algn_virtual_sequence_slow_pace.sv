...
//             num_tx_item = rx_sequence.item.data.size() / p_sequencer.model.reg_block.CTRL.SIZE.get_mirrored_value();
            
//             `uvm_do_on_with(tx_sequence, p_sequencer.md_tx_sequencer, {
//               num_tx_item == 1								       -> item.response == CFS_MD_OKAY;
//               num_tx_item > 1 && tx_item_idx < (num_tx_item - 1) -> item.response == CFS_MD_OKAY;
//               num_tx_item > 1 && tx_item_idx < (num_tx_item - 1) -> item.response == CFS_MD_ERR;
//             })
            
//             tx_item_idx++;
//           end while(tx_item_idx < num_tx_item);
//         end
//     join
//   endtask
